...
export default function OverviewView({ useCasesData, stateApiData, enrolledFarmers, farmersUpdated }) {
  ...
  const inProgress = useCasesData.filter(u => u.status === 'In Progress').length
  const complete   = useCasesData.filter(u => u.status === 'Complete').length
  const onHold     = useCasesData.filter(u => u.status === 'On Hold').length
  const pending    = useCasesData.filter(u => u.status === 'Pending').length
  const statesActive = stateApiData.filter(s => s.status === 'Active').length
  ...
  const categoryData = [
    { name: 'Central', value: useCasesData.filter(u => u.category === 'Central').length, color: '#166340' },
    { name: 'State',   value: useCasesData.filter(u => u.category === 'State').length,   color: '#3aaa75' },
    { name: 'Multi',   value: useCasesData.filter(u => u.category === 'Multi').length,   color: '#a8dfc0' },
    { name: 'Private', value: useCasesData.filter(u => u.category === 'Private').length, color: '#fef3c7' },
  ]

  const topLiveUC = useCasesData.filter(u => u.status === 'Live').slice(0, 6)
  const recentProgress = useCasesData.filter(u => u.status === 'In Progress').slice(0, 5)
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-8">
              ...
              <div className="text-right px-4 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.1)' }}>
                <div className="text-xs text-green-300 mb-0.5">
                  ...
                  <span className="block text-[10px] opacity-60 font-normal">
                    last updated on {farmersUpdated || '—'}
                  </span>
                </div>
                <div className="text-3xl font-bold text-white">{enrolledFarmers || '—'}</div>
              </div>
              <div className="text-right px-4 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.1)' }}>
                <div className="text-xs text-green-300 mb-0.5">DCS States</div>
                <div className="text-3xl font-bold" style={{ color: '#a8dfc0' }}>
                  {stateApiData.filter(s => s.dcs).length}
                </div>
              </div>
            </div>
            {/* Detail button */}
              ...
              {recentProgress.map(uc => (
                <div key={uc.id} className="flex items-center justify-between py-2 border-b border-gray-50">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                    style={{ background: uc.priority === 'High' ? '#dcfce7' : '#f3f4f6',
        ...
        <div className="bg-white rounded-xl border p-5" style={{ borderColor: '#e8f0e0' }}>
          ...
          <div className="grid grid-cols-4 gap-3">
            {stateApiData.filter(s => s.status === 'Active').slice(0, 8).map(s => (
              <div key={s.state} className="rounded-lg p-3 text-xs" style={{ background: '#f7fbf4' }}>
                ...
                <div className="flex items-center gap-1 mb-1">
                  <div className="flex-1 bg-gray-100 rounded-full h-1.5">
                    <div className="h-1.5 rounded-full" style={{ background: '#3aaa75',
