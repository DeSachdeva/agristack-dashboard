...
export default function StatesPage() {
  const active   = STATE_API_DATA.filter(s => s.status === 'Active').length
  const partial  = STATE_API_DATA.filter(s => s.status === 'Partial').length
  const mouOnly  = STATE_API_DATA.filter(s => s.status === 'MoU Only').length
  const notLive  = STATE_API_DATA.filter(s => s.status === 'Not Live' || s.status === 'Pending').length
